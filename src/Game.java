...
public class Game extends JPanel{
    ...
    int plane_x,plane_y;    //x and y location of user's car
    ...
    public Game(){
        ...
        plane_x = plane_y = 300;    //initialling setting the user's car location to (300,300)
    ...
    public void paint(Graphics g){
        ...
        try{
            ...
            obj.drawImage(getToolkit().getImage("images/ref2.gif"),plane_x,plane_y,this);   //draw car on window
    ...
    void moveRoad(int count){
        ...
        plane_x += speedX; //update car x position
        plane_y += speedY; //update car y position
        
        //case analysis to restrict car from going outside the left side of the screen
        if(plane_x < 0)   //if the car has reached or gone under its min x axis value
            plane_x = 0;  //keep it at its min x axis value
        
        //case analysis to restrict car from going outside the right side of the screen
        if(plane_x+93 >= 500) //if the car has reached or gone over its max x axis value
        ...
        if(plane_y <= 124)    //if the car has reached the the right side of the road or is trying to go further
            plane_y = 124;    //keep the car where it is
        
        //case analysis to restrict car from going outside the left side of the road
        if(plane_y >= 364-50) //if the car has reached the the left side of the road or is trying to go further
            plane_y = 364-50; //keep the car where it is
        ...
        for(int i=0;i<nOpponent;i++){ //for all opponent cars
            diff = plane_y - ly[i]; //diff is the distance between the user's car and the opponent car
            if((ly[i] >= plane_y && ly[i] <= plane_y+46) || (ly[i]+46 >= plane_y && ly[i]+46 <= plane_y+46)){   //if the cars collide vertically
                if(plane_x+87 >= lx[i] && !(plane_x >= lx[i]+87)){  //and if the cars collide horizontally
    ...
    public static void main(String args[]){
        ...
        while(true){
            ...
            if(game.nOpponent < 4 && count % 200 == 0){ //if there is less than 4 cars and count timer reaches 200
                game.imageLoc[game.nOpponent] = "images/lawan_"+((int)((Math.random()*100)%3)+1)+".gif"; //assign images to the opponent cars
